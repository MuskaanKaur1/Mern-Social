...
const create = async (req, res) => {
  console.log("REQ BODY (SIGNUP):", req.body)  // <-- logs incoming data

  const user = new User(req.body)
  try {
    const savedUser = await user.save()       // <-- store result in a variable
    console.log("SAVED USER:", savedUser)     // <-- logs the full user object

    return res.status(200).json({
      message: "Successfully signed up!"
    })
  } catch (err) {
    console.log("SAVE ERROR:", err)           // <-- logs any save error
